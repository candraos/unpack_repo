...
class ShipmentService with ChangeNotifier{
  ...
  Future getShipments() async{
    List<Shipment> shipmentsTemp = [];
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String token = prefs.getString('token') ?? '';
    String id = prefs.getString('id') ?? '';
    String type = prefs.getString('type') ?? '';
    String url = type.toLowerCase() == "employee" ? '${Constants.BASE_URL}shipments/employee-warehouse?employeeId=$id' : '${Constants.BASE_URL}getShipments?userId=$id';
    ...
    decodedBody.forEach((shipmentJson) {

      shipmentsTemp.add(Shipment.fromJson(shipmentJson));
  ...
  Future updateStatus(String id,String newStatus) async{
    ...
    final response = await http.put(
      Uri.parse('${Constants.BASE_URL}updateStatus/$id'),
      headers: <String, String>{
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json; charset=UTF-8',
      },
    ...
    if (response.statusCode == 200) {
      List<Shipment> shipmentsTemp = shipments;
      Shipment shipment = shipmentsTemp.firstWhere((shipment) => shipment.id == id);
  ...
  Future updateExpectedDeliveryDate(String id,DateTime newExpirationDate) async{
